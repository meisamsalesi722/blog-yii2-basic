...
<?php if (!Yii::$app->user->isGuest): ?>
    <button class="btn <?= $isFavorite ? 'btn-danger' : 'btn-outline-danger' ?> favorite-btn" 
        ...
        <span class="favorite-text"><?= $isFavorite ? 'حذف از علاقه‌مندی‌ها' : 'افزودن به علاقه‌مندی‌ها' ?></span>
    </button>
<?php endif; ?>
...
function toggleFavorite(articleId, btn) {
    $.ajax({
        url: '/favorite/toggle?id=' + articleId,
        type: 'POST',
        dataType: 'json',
...
<?php if (!Yii::$app->user->isGuest): ?>
    <button class="btn <?= $isFavorite ? 'btn-danger' : 'btn-outline-danger' ?> favorite-btn" 
        ...
        <span class="favorite-count badge bg-light text-dark ms-1">
            <?= \app\models\Favorite::find()->where(['article_id' => $model->id])->count() ?>
        </span>
    </button>
<?php endif; ?>
